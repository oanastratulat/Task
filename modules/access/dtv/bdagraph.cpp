...
HRESULT BDAGraph::CreateTuneRequest()
{
   ...
   * network type then all is well. Otherwise, reset the Tuning Space and ge
   * a new one */
  if( p_tuning_space )
  {
...
HRESULT BDAGraph::Build()
{
   ...
   * its Scanning Tuner which takes the Tune Reques
   ...
   * If it is not what was selected then the requested device was no
 ...
 * Obvious candidate is FindFilter */
  ...
  /* Always look for the Transform Information Filter from the star
   * of the collection*/
  l_tif_used = -1;
...
HRESULT BDAGraph::Connect( IBaseFilter* p_upstream, IBaseFilter* p_downstream )
{
  ...
  do
  {
    ...
    do
    {
      ...
      /* If we arrive here it means this downstream pin is no
