...
static void* WINAPI expWaitForSingleObject(void* object, int duration)
{
 ...
 return (void*)WAIT_FAILED;
    ...
    // this object really exists in our lis
    if (!ml)
 return (void*) ret;
...
#ifndef QTX
    /* 10l to QTX, if CreateMutex returns a real mutex, WaitForSingleObjec
 ...
 *s='\0';
  ...
  * some problems on WaitForSingleObject,CreateEvent,SetEven
  * CreateThread ...etc..
  *
  */
 cachedsi.dwNumberOfProcessors=1;
...
{
    // The state of a semaphore object is signaled when its coun
...
/*
 * Returns the number of milliseconds, modulo 2^32, since the star
 * of the wineserver.
 */
...
/*
 * local definition - we need only the last two members at this poin
...
static void exp_ftol(void)
{
 ...
 (
  ...
  "fldcw   -2(%ebp) \n\t"
  "movl  -12(%ebp), %eax \n\t"
  //Note: gcc 3.03 does not do the following op if i
  //      knows that ebp=esp
  "movl %ebp, %esp       \n\t"
 );
}
...
struct exports exp_pncrt[]={
    FF(malloc, -1) // just tes
    FF(free, -1) // just tes
    FF(fprintf, -1) // just tes
    {"_adjust_fdiv", -1, (void*)&_adjust_fdiv},
    FF(_ftol,-1)
    FF(_initterm, -1)
    {"??3@YAXPAX@Z", -1, expdelete},
