...
/* callback for atmo settings variables whose change
 should be immediately realized and applied to outpu
*/
...
/*
  instead of redefining the original AtmoLight zones with gradien
 ...
 |____________|
  ...
  the zone numbers will be counted clockwise starting at top / lef
...
struct filter_sys_
{
  ...
  */
  ...
  change event of the variable "crop" from vou
  */
...
/*
initialize previously configured Atmo Light environmen
- if internal is enabled try to access the device on the serial por
  ...
  */
  ...
  if( !EMPTY_STR(psz_serialdev) )
  {
    ...
    */
    ...
    /*
      contains string only spaces or is empty? skip i
    */
    if( !psz_token[i] )
      continue;
...
static void Atmo_SetupParameters(filter_t *p_filter)
{
  ...
  */
  ...
  if(p_sys->i_device_type == 0) {
    ...
    {
    /*
      be clever if the location of atmowina.exe is se
  ...
  */
  ...
  if(i == 1) /* Init Atmolight success... */
  {
    ...
    /*
    configure
     p_sys->i_atmo_width and p_sys->i_atmo_heigh
           ...
           uint8_t *p_transfer_dest)
{
  ...
  image so we wont get column and row - zero from the frame, and not the mos
  right and bottom pixels --- which may be clipped on computers useing TV ou
  - through overscan!
  ...
  */
  for(i_row = 1; i_row < i_row_count; i_row++)
  {
    ...
    */
    ...
    i_pixel_row = (i_row * p_sys->i_crop_height) / i_row_coun
...
* to a target color defined in p_fadethread struc
    ...
    */
      ...
      (i_steps_done < p_fadethread->i_steps))
    {
      ...
      */
      ...
      the VLC libaries? inside native win32 I would use an Even
         ...
         void *p_data )
{
  ...
  if(p_sys->b_usepausecolor && p_sys->b_enabled)
  {
    ...
    {
    /* tell the other thread to stop sending images to ligh
     controller */
    p_sys->b_pause_live = true;
