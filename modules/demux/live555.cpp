...
class RTSPClientVlc : public RTSPClien
{
public:
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
  ...
  switch( i_query )
  {
    ...
    *pb2 = false;
    ...
    /* According to RFC 2326 p56 chapter 12.35 a RTSP server tha
     * supports Scale:
     *
     * "[...] should try to approximate the viewing rate, bu
    ...
    /* Passing -1 for the start and end time will mean liveMedia won'
    ...
    *pi_int = (int)( INPUT_RATE_DEFAULT / p_sys->ms->scale() );
     ...
     * Unfortunately Live555 isn't thread safe and so can'
            ...
            const uint8_t *p_data, unsigned i_size )
{
  ...
  while( i_size >= 4 )
  {
    ...
    /* XXX
   * When b_length is true, the streams I found do not seems to respec
   * the documentation.
        ...
        unsigned int duration )
{
  ...
  /* grow buffer if it looks like buffer is too small, but don't ea
