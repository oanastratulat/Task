...
std::string dvd_chapter_codec_c::GetCodecName( bool f_for_title ) cons
{
  std::string result;
  if ( p_private_data->GetSize() >= 3)
  {
...
bool dvd_command_interpretor_c::Interpret( const binary * p_command, size_t i_size )
{
  ...
  case CMD_DVD_JUMPVTS_PTT:
    ...
    // find the current VTS content segmen
...
// see http://www.matroska.org/technical/specs/chapters/index.html#mscrip
