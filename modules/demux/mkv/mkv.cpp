 ...
 * $Id: a350a43af3df9301bd6f5480df350231c2c57d2e $
 *
         ...
         mtime_t i_pts, mtime_t i_duration, bool f_mandatory )
{
   ...
   i++ )
  {
    ...
    if ( tk->fmt.i_cat == NAV_ES )
    {
    // TODO handle the start/stop times of this packe
    ...
    if ( p_vsegment->CurrentEdition() && p_vsegment->CurrentEdition()->b_ordered )
    {
      ...
      if ( p_chap != NULL )
      {
        ...
        */
        ...
        p_sys->i_pts++; // trick to avoid staying on segments with no duration and no conten
