 ...
 * $Id: 83edcb7a53b57653e272fb279f9d815276a83884 $
 *
...
matroska_stream_c *demux_sys_t::AnalyseAllSegmentsFound( demux_t *p_demux, EbmlStream *p_estream, bool b_initial )
{
  ...
  while (p_l0 != 0)
  {
    ...
    while ((p_l1 = ep->Get()))
    {
      ...
      {
        // find the families of this segmen
...
bool demux_sys_t::PreloadLinked()
{
  ...
  // publish all editions of all usable segmen
  for ( i=0; i< used_segments.size(); i++ )
  {
...
virtual_segment_c *demux_sys_t::VirtualFromSegments( std::vector<matroska_segment_c*> *p_segments ) cons
{
  if ( p_segments->empty() )
    return NULL;
...
void demux_sys_t::JumpTo( virtual_segment_c & vsegment, virtual_chapter_c * p_chapter )
{
  ...
  if ( p_chapter )
  {
    ...
    {
    // jump to the location in the found segmen
...
matroska_segment_c *demux_sys_t::FindSegment( const EbmlBinary & uid ) cons
{
  for (size_t i=0; i<opened_segments.size(); i++)
  {
