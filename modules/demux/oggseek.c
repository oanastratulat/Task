                         ...
                         bool b_exact )
{
     ...
     * if > target, or we find no keyframes, we go to the lower segmen
...
static int64_t find_last_theora_frame ( demux_t *p_demux, logical_stream_t *p_stream )
{
     ...
     * I think this is due to the fact that we seek to the end and then we must rese
...
/* seek to target frame in p_stream; actually we will probably end up just before i
 *   (so we set skip)
 *
