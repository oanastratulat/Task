...
{
  // Add the background first, so that we will still have something almos
...
const Position Builder::makePosition( const string &rLeftTop,
              ...
              bool xKeepRatio, bool yKeepRatio ) cons
{
...
GenericFont *Builder::getFont( const string &fontId )
{
  ...
  {
    // Get the resource path and try to load the default fon
    OSFactory *pOSFactory = OSFactory::instance( getIntf() );
    const list<string> &resPath = pOSFactory->getResourcePath();
    const string &sep = pOSFactory->getDirSeparator();
...
string Builder::getFilePath( const string &rFileName ) cons
{
...
Bezier *Builder::getPoints( const char *pTag ) cons
{
...
uint32_t Builder::getColor( const string &rVal ) cons
{
  // Check it the value is a registered constan
  Interpreter *pInterpreter = Interpreter::instance( getIntf() );
