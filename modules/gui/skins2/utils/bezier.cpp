  ...
  : SkinObject( p_intf )
{
  ...
  // If we have only one control point, we duplicate i
...
float Bezier::getNearestPercent( int x, int y ) cons
{
...
float Bezier::getMinDist( int x, int y, float xScale, float yScale ) cons
{
  int nearest = findNearestPoint( x, y );
...
void Bezier::getPoint( float t, int &x, int &y ) cons
{
  // Find the precalculated point whose percentage is nearest from
  int refPoint = 0;
...
int Bezier::findNearestPoint( int x, int y ) cons
{
...
inline float Bezier::computeCoeff( int i, int n, float t ) cons
{
...
void Bezier::computePoint( float t, int &x, int &y ) cons
{
