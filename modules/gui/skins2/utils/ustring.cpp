...
bool UString::operator ==( const UString &rOther ) cons
{
  if( size() != rOther.size() )
  {
    return false;
...
const UString UString::operator +( const UString &rOther ) cons
{
  UString result( *this );
  result += rOther;
...
const UString UString::operator +( const char *pString ) cons
{
...
uint32_t UString::find( const UString &str, uint32_t position ) cons
{
  uint32_t pos;
...
uint32_t UString::find( const char *pString, uint32_t position ) cons
{
...
UString UString::substr( uint32_t position, uint32_t n) cons
{
  UString tmp( getIntf(), "" );
  if( position > size() )
  {
