 ...
 * $Id: d395626afb1fef8d96a129f51566a91f42493aee $
 *
         ...
         tree_update *arg )
{
  ...
  else if( arg->type == arg->DeletingItem )
  {
    ...
    // remove all references to arg->i
...
bool CtrlTree::mouseOver( int x, int y ) cons
{
  const Position *pPos = getPosition();
  return !pPos ? false :
...
void CtrlTree::makeImage()
{
  ...
  for( int yPos = 0; yPos < height && it != m_rTree.end(); ++it )
  {
    ...
    if( m_pCurBitmap )
    {
      ...
      // Draw the icon in front of the tex
...
bool CtrlTree::isItemVisible( const Iterator& it_ref )
{
  ...
  // Ensure a partially visible last item is taken into accoun
