...
bool CtrlSliderCursor::mouseOver( int x, int y ) cons
{
  if( m_pImg )
  {
...
bool CtrlSliderBg::mouseOver( int x, int y ) cons
{
  // Compute the resize factors
  float factorX, factorY;
  getResizeFactors( factorX, factorY );
...
void CtrlSliderBg::getResizeFactors( float &rFactorX, float &rFactorY ) cons
{
  // Get the position of the control
  const Position *pPos = getPosition();
