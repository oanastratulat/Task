...
bool CtrlText::mouseOver( int x, int y ) cons
{
  if( !m_pFocus->get() )
    return false;
  ...
  if( m_pCurrImg )
  {
    ...
    //  3) the mouse is over the rectangle of the visible tex
...
{
  // Create the images ('normal' and 'double') from the tex
  // 'Normal' image
  delete m_pImg;
...
void CtrlText::onPositionChange()
{
  if( m_pImg && getPosition() )
  {
    ...
    // make sure to stop any scrolling effec
    m_pTimer->stop();
    m_xPos = 0;
    }
    else
