  ...
  m_dragDrop( dragDrop ), m_pDropTarget( NULL ), m_type ( type )
{
  ...
  if( m_dragDrop )
  {
    ...
    // Register the window as a drop targe
...
void X11Window::hide() cons
{
  // Unmap the window
  XUnmapWindow( XDISPLAY, m_wnd );
}

void X11Window::moveResize( int left, int top, int width, int height ) cons
{
  if( width && height )
...
void X11Window::raise() cons
{
  XRaiseWindow( XDISPLAY, m_wnd );
}


void X11Window::setOpacity( uint8_t value ) cons
{
...
void X11Window::setFullscreen( ) cons
{
  if( NET_WM_STATE_FULLSCREEN != None )
  {
...
void X11Window::toggleOnTop( bool onTop ) cons
{
...
bool X11Window::invalidateRect( int x, int y, int w, int h ) cons
{
