...
const list<LayeredControl> &GenericLayout::getControlList() cons
{
             ...
             int xOffSet, int yOffSet )
{
  ...
  if( width > 0 && height > 0 )
  {
    // make sure region is within the layou
...
void GenericLayout::resize( int width, int height )
{
  ...
  if( pWindow )
  {
    ...
    // Change the shape of the window and redraw i
    refreshAll();
  }
}
...
void GenericLayout::refreshRect( int x, int y, int width, int height )
{
  ...
  // Draw all the controls of the layou
  list<LayeredControl>::const_iterator iter;
  for( iter = m_controlList.begin(); iter != m_controlList.end(); ++iter )
  {
    CtrlGeneric *pCtrl = (*iter).m_pControl;
...
const list<Anchor*>& GenericLayout::getAnchorList() cons
{
  return m_anchorList;
}
