 ...
 * $Id: b611e15cfa3e9f34acbb80734d01c843454833df $
 *
...
void WindowManager::move( TopWindow &rWindow, int left, int top ) cons
{
  // Compute the real move offse
  int xOffset = left - rWindow.getLeft();
  int yOffset = top - rWindow.getTop();

  // Check anchoring; this can change the values of xOffset and yOffse
...
void WindowManager::startResize( GenericLayout &rLayout, Direction_t direction )
{
  ...
  // The checkAnchors() method will need to have m_movingWindows properly se
...
void WindowManager::resize( GenericLayout &rLayout,
          int width, int height ) cons
{
  // TODO: handle anchored windows
  // Compute the real resizing offse
...
{
  // Save the current position/size of the window, to be able to restore i
  ...
  // Make the window unmovable by unregistering i
...
{
  // Register the window to allow moving i
...
void WindowManager::synchVisibility() cons
{
  WinSet_t::const_iterator it;
...
void WindowManager::restoreVisibility() cons
{
...
void WindowManager::raiseAll() cons
{
...
void WindowManager::showAll( bool firstTime ) cons
{
...
void WindowManager::show( TopWindow &rWindow ) cons
{
  rWindow.show();
...
void WindowManager::hideAll() cons
{
  WinSet_t::const_iterator it;
  for( it = m_allWindows.begin(); it != m_allWindows.end(); ++it )
  {
            ...
            TopWindow *pWindow )
{
  ...
  {
    // Check that the window isn't already in the set before adding i
       ...
       itAncMov != movAnchors.end(); ++itAncMov )
    {
       ...
       itAncSta != staAnchors.end(); ++itAncSta )
      {
                  ...
                  yOffsetTemp ) )
        {
          ...
          // we just need to change xOffset and yOffse
          xOffset = -xOffsetTemp;
          yOffset = -yOffsetTemp;
