...
void TopWindow::processEvent( EvtMouse &rEvtMouse )
{
  ...
  if( rEvtMouse.getAction() == EvtMouse::kDown )
  {
    ...
    {
    // If a new control gains the focus, the previous one loses i
...
const GenericLayout& TopWindow::getActiveLayout() cons
{
...
void TopWindow::onControlRelease( const CtrlGeneric &rCtrl )
{
  ...
  // Send an enter event to the control under the mouse, if it doesn'
  // have received it ye
...
CtrlGeneric *TopWindow::findHitControl( int xPos, int yPos )
{
  ...
  // Get the controls in the active layou
  ...
  {
    // Get the position of the control in the layou
  ...
  // If the hit control has just been entered, send it an enter even
