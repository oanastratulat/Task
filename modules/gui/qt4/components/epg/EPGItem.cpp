...
QRectF EPGItem::boundingRect() cons
{
...
void EPGItem::paint( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget*)
{
  ...
  // Get the transformations required to map the text on the viewpor
...
const QDateTime& EPGItem::start() cons
{
  return m_start;
}
...
bool EPGItem::endsBefore( const QDateTime &ref ) cons
{
  return m_start.addSecs( m_duration ) < ref;
}

bool EPGItem::playsAt( const QDateTime & ref ) cons
{
