 ...
 * $Id: 8b3e089282067204a80179e44f8f7362e6a5e6cc $
 *
...
Qt::DropActions PLModel::supportedDropActions() cons
{
  return Qt::CopyAction | Qt::MoveAction;
}

Qt::ItemFlags PLModel::flags( const QModelIndex &index ) cons
{
...
QStringList PLModel::mimeTypes() cons
{
  QStringList types;
...
QMimeData *PLModel::mimeData( const QModelIndexList &indexes ) cons
{
...
QVariant PLModel::data( const QModelIndex &index, const int role ) cons
{
...
bool PLModel::isParent( const QModelIndex &index, const QModelIndex &current ) cons
{
  if( !index.isValid() )
    return false;
...
bool PLModel::isCurrent( const QModelIndex &index ) cons
{
  return getItem( index )->inputItem() == THEMIM->currentInputItem();
}

int PLModel::itemId( const QModelIndex &index ) cons
{
...
QModelIndex PLModel::index( PLItem *item, int column ) cons
{
...
QModelIndex PLModel::currentIndex() cons
{
...
QModelIndex PLModel::parent( const QModelIndex &index ) cons
{
...
int PLModel::columnCount( const QModelIndex &) cons
{
  return columnFromMeta( COLUMN_END );
}

int PLModel::rowCount( const QModelIndex &parent ) cons
{
...
/************************* Lookups *****************************/
PLItem *PLModel::findById( PLItem *root, int i_id ) cons
{
  return findInner( root, i_id, false );
}

PLItem *PLModel::findByInput( PLItem *root, int i_id ) cons
{
  PLItem *result = findInner( root, i_id, true );
  return result;
}

PLItem * PLModel::findInner( PLItem *root, int i_id, bool b_input ) cons
{
  if( !root ) return NULL;
...
QList<input_item_t*> PlMimeData::inputItems() cons
{
  return _inputItems;
}

QStringList PlMimeData::formats () cons
{
  QStringList fmts;
