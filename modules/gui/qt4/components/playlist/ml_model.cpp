 ...
 * $Id: 3aae7ddb59ed66c3ee0fd3dabe9c843c2652d4da $
 *
 ...
 * @param parent the parent Qt objec
 */
...
QModelIndex MLModel::parent(const QModelIndex & ) cons
{
  return QModelIndex();
}
 ...
 */
QModelIndex MLModel::currentIndex() cons
{
 ...
 */
ml_select_e MLModel::columnType( int logicalindex ) cons
{
...
Qt::ItemFlags MLModel::flags(const QModelIndex &index) cons
{
  if( !index.isValid() )
    return 0;
...
bool MLModel::isEditable( const QModelIndex &index ) cons
{
  if( !index.isValid() )
    return false;
...
QMimeData* MLModel::mimeData( const QModelIndexList &indexes ) cons
{
  QList< QUrl > urls;
  QList< int > rows;
...
int MLModel::columnCount( const QModelIndex & ) cons
{
  return columnFromMeta( COLUMN_END );
}

int MLModel::rowCount( const QModelIndex & parent ) cons
{
  if( !parent.isValid() )
    return items.count();
  return 0;
...
int MLModel::getId( QModelIndex index ) cons
{
  return getItem( index )->id();
}

QVariant MLModel::data( const QModelIndex &index, const int role ) cons
{
  if( index.isValid() )
  {
...
/** **************************************************************************
 * \brief Add a media to the playlis
 *
