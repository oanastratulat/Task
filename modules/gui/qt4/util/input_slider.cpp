 ...
 * $Id: 382cfb4551c3d7752c3a0ce68c78b11dba2ccf31 $
 *
...
QSize SeekSlider::sizeHint() cons
{
  return ( orientation() == Qt::Horizontal ) ? QSize( 100, 18 )
                 : QSize( 18, 100 );
}

QSize SeekSlider::handleSize() cons
{
...
void SeekSlider::paintEvent( QPaintEvent *event )
{
  ...
  // set the background color and gradien
  ...
  // set the foreground color and gradien
  ...
  if ( option.state & QStyle::State_MouseOver || isAnimationRunning() )
  {
    ...
    // prepare the handle's shadow gradien
...
qreal SeekSlider::handleOpacity() cons
{
...
bool SeekSlider::isAnimationRunning() cons
{
