...
bool PluginTreeItem::operator< ( const QTreeWidgetItem & other ) cons
{
...
int ExtensionListModel::rowCount( const QModelIndex& ) cons
{
  int count = 0;
  ExtensionsManager *EM = ExtensionsManager::getInstance( p_intf );
  extensions_manager_t *p_mgr = EM->getManager();
...
QVariant ExtensionListModel::data( const QModelIndex& index, int role ) cons
{
  if( !index.isValid() )
    return QVariant();
